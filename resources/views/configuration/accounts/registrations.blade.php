...
<div class="gs-portal" style="padding:24px">
    ...
    <div class="gs-card"><div class="gs-table-wrap"><table class="gs-table"><thead><tr><th>Name</th><th>Username</th><th>Email</th><th>Student No.</th><th>Registered</th><th>Status</th><th>Action</th></tr></thead><tbody>@forelse($accounts as $account)<tr><td>{{ $account->name ?: $account->username }}</td><td>{{ $account->username }}</td><td>{{ $account->email }}</td><td>{{ $type === 'student' ? ($account->student?->student_number ?? 'Not linked') : '-' }}</td><td>{{ $account->created_at?->format('M d, Y') }}</td><td><span class="gs-badge" data-status="{{ $account->status }}">{{ $account->status }}</span></td><td><a class="gs-btn" href="{{ route('configuration.accounts.registrations.show', [$type, $account->id]) }}">Review</a></td></tr>@empty<tr><td colspan="7">No registrations found.</td></tr>@endforelse</tbody></table></div></div>
