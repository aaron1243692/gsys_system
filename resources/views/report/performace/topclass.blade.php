        ...
        <section class="flex w-full flex-1 min-h-0">
            <div class="flex w-full flex-1 flex-col rounded-lg border border-slate-200 bg-white px-2 py-3 shadow-sm min-h-0">
                <div class="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <form method="GET" action="{{ route('report.performance.top-class') }}" class="flex w-full gap-2 md:max-w-md">
                        <input
                            type="search"
                            name="search"
                            value="{{ $search }}"
                            placeholder="Search class, grade level, or year"
                        ...
                        <tbody>
                            @forelse ($classes as $class)
                                <tr class="border-b border-slate-200 hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-2 font-semibold text-slate-700">
                                        {{ $class->class_name }}
                                        <div class="text-xs font-semibold text-slate-500">{{ $class->grade_level_name ?? 'No grade level' }}</div>
                                    </td>
                                    <td class="px-4 py-2 font-semibold text-slate-700">{{ $class->academic_year_name ?? '-' }}</td>
                                    <td class="px-4 py-2 font-semibold text-slate-700">{{ $class->student_count }}</td>
                                    <td class="px-4 py-2">
