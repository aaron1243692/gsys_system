        ...
        <div class="gs-public-body">
            ...
            <form id="registration-form" method="POST" action="{{ route('portal.register.store', ['portal' => $portal]) }}" class="gs-panel">
                ...
                <div class="gs-grid">
                    ...
                    <label class="gs-field">Contact Number @if($isStudent)<span class="gs-required">Required</span>@endif<input name="contact" value="{{ old('contact') }}" maxlength="100" @required($isStudent)>@error('contact')<span class="gs-error" role="alert">{{ $message }}</span>@enderror</label>
                    <label class="gs-field gs-field-wide">Address @if($isStudent)<span class="gs-required">Required</span>@endif<textarea name="address" rows="3" maxlength="255" @required($isStudent)>{{ old('address') }}</textarea>@error('address')<span class="gs-error" role="alert">{{ $message }}</span>@enderror</label>
                    @if($isStudent)
                        <label class="gs-field">LRN <span class="gs-required">Required</span><input type="text" name="lrn" value="{{ old('lrn') }}" required inputmode="numeric" pattern="[0-9]{12}" minlength="12" maxlength="12" autocomplete="off">@error('lrn')<span class="gs-error" role="alert">{{ $message }}</span>@enderror</label>
