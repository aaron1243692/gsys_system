...
@php($name = $portal === 'student' ? ($identity->info?->name ?? $identity->username) : ($identity->name ?: $identity->username))
    ...
    <div class="gs-card-body">
        ...
        <div class="gs-grid">
            ...
            @if(in_array($portal, ['student','guardian'], true))
                <label class="gs-field">Contact<input name="contact" value="{{ old('contact', $portal === 'student' ? $identity->info?->contact : $identity->contact) }}"></label>
                <label class="gs-field gs-field-wide">Address<textarea name="address" rows="3">{{ old('address', $portal === 'student' ? $identity->info?->address : $identity->address) }}</textarea></label>
            @endif
